...
var Particles = (function(window, document) {
  ...
  // Global variables
    //LM
  var Plugin, Particle, ParticleUser,canvas, context, config;
  ...
  Plugin = function() {
    ...
    // LM
    this.ponerParticula=function(pX,pY)
      {
        this.storage.push(new ParticleUser(pX,pY));
      }
    /**
...
// LM
    ParticleUser = function(pX,pY) {
        // Setup random attributes
        this.x = pX;
        this.y = pY;
        this.vx = Math.random() * config.speed * 2 - config.speed;
        this.vy = Math.random() * config.speed * 2 - config.speed;
        this.radius =Math.random()*config.sizeVariations;
