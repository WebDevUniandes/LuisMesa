...
function Point(x, y, z, size, color, texto)
{
    ...
    this.update = function ()
    {
        ...
        //LM Permite que se cambie el eje z
        // if(ejeZ)
        // {
    ...
    this.draw = function (bubbleShape, dx, dy)
    {
        ...
        else
        {
            ...
            var fontSize = Math.floor(this.radius / 3);
            var texto = this.texto;
            ctx.font = 'bold ' + fontSize + 'px Arial';
            ctx.fill();
            ctx.fillStyle = "white";
            ctx.fillText(texto, (this.curPos.x + dx) - (fontSize * texto.length / 4), this.curPos.y + dy + (fontSize / 4));
...
{
    // LM Se trae al elemento padre del canvas y se le pide el ancho y alto para usarlo en el canvas.
    var div =document.getElementById('habilidades');
    var extra = (document.getElementById('colVacia1').offsetWidth)*2;

    var h=div.offsetHeight.toString();
    var w;

    if((extra+div.offsetWidth)<=self.innerWidth/2)
    {
        w = (extra + div.offsetWidth).toString();
    }
    else
    {
        w=div.offsetWidth.toString();
    }
...
function drawCircles(lenguajes)
{
    //LM
    updateCanvasDimensions();
    var g = [];
    ...
    function addLetter(letterCols, texto, posiciones,tamanio,color)
    {
        if (Object.prototype.toString.call(letterCols) === '[object Array]' && Object.prototype.toString.call(letterCols[0]) === '[object Array]')
        {
            letterColors = letterCols;
        ...
        //x,y,z,size,color,texto
        var size = Math.floor(40+(Math.random()*20));
        var posX= Math.floor((Math.random()*100));
        var posY= Math.floor((Math.random()*100));
        var posZ= Math.floor((Math.random()*40));
        // if(g.length==0)
        // {
            g.push(new Point(posiciones[0],posiciones[1], 0.0, tamanio, makeColor(color, size),texto));
        // }
        // else
        // {   if(Math.random()<0.5)
        //         posX=-posX;
        ...
        //         posX=-posY;
        //
        //     g.push(new Point(posX, posY, 0.0, size, makeColor(letterCols[Math.floor((Math.random() * letterCols.length))], size),texto));
        // }


        // g.push(new Point(50, 50, 0.0, size, makeColor(letterCols[Math.floor((Math.random() * letterCols.length))], size)));
        // g.push(new Point(0, 0, 0.0, size, makeColor(letterCols[Math.floor((Math.random() * letterCols.length))], size),texto));
        // offset += document.alphabet[cc_hex].W;

    }

    var numCirculos = lenguajes.length;


    for (var i = 0; i < numCirculos; i ++)
    {
        addLetter(letterColors,lenguajes[i].nombre, lenguajes[i].posicion,lenguajes[i].tamanio,lenguajes[i].color);
    }
...
var canvas = $("#canvasLenguajes");
var canvasHeight;
var canvasWidth;
var ctx;
var pointCollection;
//LM boolean que activa el eje z
var ejeZ=false;
